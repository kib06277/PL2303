...
public class MainActivity extends AppCompatActivity
{
    ...
    public void onResume() {
        ...
        if(!mSerial.isConnected()) {
            ...
            Log.d(TAG, "onResume fail Connected");
...
//        if(i.substring(0 , 4).equals("AAAA"))
//        {
//            Log.d(TAG, "T");
//            rbuf = new byte[12];
//        }
...
//        }
        ...
        if (len > 0) {
            ...
            //rbuf[len] = 0;
            sbHex.append(bytesToHexString(rbuf));
//            for (int j = 0; j < len; j++) {
...
//        }
    ...
    private void writeDataToSerial() {
        ...
        String strWrite = etWrite.getText().toString();
//        byte[] data = new BigInteger(strWrite, 16).toByteArray();
        byte[] data = hexToBytes(strWrite);
        Log.d(TAG, "bytesToHexString : " + bytesToHexString(data));
...
//    public void CRC16()
//    {
//        HashMap rs = MainActivity.plModel.ReadCurrent();
//        if (null != rs)
//        {
//            // 從 EE 讀取比對資料正確性
//            // 先判斷 Device ID 跟 CheckSum, 不一樣就問是否要覆蓋或捨棄
//            boolean isSameDevice = true;
//            String[] compasDid = MAPDataWrite.getDeviceID().split(",");
//            int[] compasChkSum = MAPDataWrite.getCheckSum();
//
//            Log.d(TAG, "compasDid = " + compasDid + " compasChkSum = " + compasChkSum);
//
//            plModel.Reset();
//
//            byte[] did = plModel.ReadRandom(SGPL2303MAPData.OT_IDCode_0, 0x08).get(plModel.PL2303_ACK_OK);
//            byte[] ckSum = plModel.ReadRandom(SGPL2303MAPData.OT_MAPCheckSum_1, 0x02).get(plModel.PL2303_ACK_OK);
//
//            if (did.length > 0 && ckSum.length > 0)
//            {
//                // 判斷識別碼
//                for (int di = 0; di < compasDid.length; di++)
//                {
//                    if (Integer.decode(compasDid[di]) != (int) did[di])
//                    {
//                        isSameDevice = false;
//                        break;
//                    }
...
//
//                // 判斷 CheckSum
//                for (int di = 0; di < compasChkSum.length; di++)
//                {
//                    char c = (char) (ckSum[di] & 0xFF);
//                    if ((compasChkSum[di] & 0xFF) != c)
//                    {
//                        //Toast.makeText(ACTSetDeviceOption.this,
//                        //        "ckSum " + (compasChkSum[di] & 0xFF) + ":" + c,
